...
The canonical repo is `github.com/steadman-ai/expedia-cbb` (branches `main` and `vault-build`, full history, kept current since July 2026) — clone from there, and every change must land there. It does not trigger deploys. Until the Railway services are re-pointed at the Steadman repo, a change only reaches production/preview when it is ALSO pushed to the matching branch of the legacy repo `willaudiencestrategies/pitch-pack-tool`, so add that as a second remote (on Will's checkout it is named `railway-legacy`; `origin` is the Steadman repo) and push both. Remember the asymmetry: pushing legacy `vault-build` is a safe staging deploy, pushing legacy `main` is a production deploy — as of 21 July 2026 legacy `main` is deliberately held back at cb2a4cf (4 June) pending the client's go-live nod, while Steadman `main` carries the full July work. [TODO: Will to hand over Railway project access, confirm both services' sources and region in the dashboard (Settings → Source), and re-point them at steadman-ai/expedia-cbb so the legacy repo can be retired.]
...
`steadman-ai/expedia-cbb` (canonical) holds everything above on both branches. Production Railway deploys from the LEGACY repo `willaudiencestrategies/pitch-pack-tool` `main` (section 8). Whether the July work is live depends on whether that legacy main has been pushed — check the legacy repo's `main` (remote `railway-legacy` on Will's checkout) against this changelog's commits before assuming. As of 21 July 2026: the **Vault Preview staging service carries all of this changelog including 19.8** (legacy `vault-build` pushed and deploy verified via bundle markers); **production does not** (legacy main still at cb2a4cf, 4 June). Going live = pushing legacy `main`.
