...
- **Word export renderer gaps.** `markdownToParagraphs` in `src/lib/word-export.ts` handles headings, bullets, numbered lists, bold/italic, horizontal rules and the Differentiator prefix, but not tables or blockquotes. If a prompt change makes the compile emit a table, the docx will contain raw pipe characters.
- **Budget can be silently dropped (Tim's bug, mechanism known, unfixed).** The budget step renders two advance paths: the `ProductionBudget` component with its own "Confirm Budget" button, and the generic section footer "Confirm & Continue" (main `src/app/page.tsx` around lines 892 and 1064). `ProductionBudget` holds currency and amounts in *local* component state (`src/components/ProductionBudget.tsx`) and only writes them into `state.budgetDetails` via its own confirm. The footer button just advances the section, so a user who types budget figures and clicks the footer loses them without any warning. The fix is to remove or wire the footer button on the budget section.
- **Brand selection friction (unreproduced).** `src/components/BrandAlignment.tsx` gates the continue button on an async brand-fit LLM check, and for moderate/weak fits additionally on an acknowledge tick (`fitAcknowledged`); strong fits auto-acknowledge. If the fit call is slow or errors, the button stays disabled, which is the most likely explanation for the "can't reliably pick the EG brand" report. Not reproduced locally.
